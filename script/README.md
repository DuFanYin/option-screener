...
# Select which strategy types to generate
...
cfg = ConfigFilter(
    ...
    # Strategy-level filters
    direction=Direction.LONG,  # or Direction.SHORT for short positions
    debit_range=(5, 15),  # Debit range
    credit_range=None,  # Credit range (for credit strategies)
...
# Use fluent API to generate strategies
results = factory.strategy(strategy_filter).config(cfg).rank("rr").top(10)
...
The factory supports a fluent API for easy chaining:

```python
# Generate strategies with filter and config
strategies = factory.strategy(strategy_filter).config(cfg)

# Rank and get top N
top_strategies = factory.strategy(strategy_filter).config(cfg).rank("rr").top(10)

# Convert to DataFrame
df = factory.strategy(strategy_filter).config(cfg).rank("cost").top(20).to_df()
```
