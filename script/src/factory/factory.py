# factory.py
from typing import Optional, Tuple, List, Callable

from ..object import Option
from .filter import ConfigFilter, StrategyFilter
from .option_filter import OptionFilter
from ..strategy.generator_class import (
    SingleCallsGenerator,
    IronCondorsGenerator,
    StraddlesGenerator,
    StranglesGenerator
)


# ranking.py
class StrategyRanking:
    @staticmethod
    def by_rr(strats):
        return sorted(strats, key=lambda strategy: strategy.rr(), reverse=True)

    @staticmethod
    def by_profit(strats):
        return sorted(strats, key=lambda strategy: strategy.max_gain(), reverse=True)

    @staticmethod
    def by_cost(strats):
        return sorted(strats, key=lambda strategy: strategy.cost())

# ============================================================
# ✅ STRATEGY FACTORY
# ============================================================

# ============= Helper Wrapper for returned results =============
class StrategyList(list):
    def __init__(self, items, factory):
        super().__init__(items)
        self._factory = factory

    def rank(self, key: str = "rr", reverse: bool = True) -> 'StrategyList':
        if not self:
            return StrategyList([], self._factory)

        # Ranking key functions
        rankers: dict[str, Callable] = {
            "rr": lambda s: s.rr(),
            "gain": lambda s: s.max_gain(),
            "loss": lambda s: s.max_loss(),
            "cost": lambda s: s.cost(),
            "credit": lambda s: s.credit() if hasattr(s, "credit") and s.direction == "SHORT" else 0,
        }

        if key in rankers:
            key_func = rankers[key]
            # Special handling for loss (ascending) and credit (only for IronCondor)
            if key == "loss":
                sorted_list = sorted(self, key=key_func)
            elif key == "credit" and (not self or not hasattr(self[0], "credit") or self[0].direction != "SHORT"):
                sorted_list = list(self)
            else:
                sorted_list = sorted(self, key=key_func, reverse=reverse)
        else:
            sorted_list = list(self)

        return StrategyList(sorted_list, self._factory)

    def top(self, n=10):
        return StrategyList(self[:n], self._factory)

    def to_df(self):
        import pandas as pd
        rows = [{
            "strategy": s.pretty(),
            "cost": s.cost(),
            "max_gain": s.max_gain(),
            "max_loss": s.max_loss(),
            "rr": s.rr(),
            "delta": s.net_delta(),
            "theta": s.net_theta(),
            "vega": s.net_vega(),
            "iv": s.avg_iv(),
        } for s in self]
        return pd.DataFrame(rows)


# ===================== FACTORY =====================
class StrategyBuilder:
    """Builder for fluent API: factory.strategy(filter).config(cfg)"""
    def __init__(self, factory: 'StrategyFactory', strategy_filter: StrategyFilter):
        self.factory = factory
        self.strategy_filter = strategy_filter
    
    def config(self, cfg: ConfigFilter) -> StrategyList:
        """Apply config and generate strategies"""
        return self.factory.generate(self.strategy_filter, cfg)


class StrategyFactory:
    def __init__(self, options: List[Option], spot: float):
        self.options = options
        self.spot = spot
        # Registry mapping strategy filter fields to generators
        self._generators = {
            'single_calls': SingleCallsGenerator(options, spot),
            'iron_condors': IronCondorsGenerator(options, spot),
            'straddles': StraddlesGenerator(options, spot),
            'strangles': StranglesGenerator(options, spot),
        }

    def strategy(self, strategy_filter: StrategyFilter) -> StrategyBuilder:
        """Start building strategies with a filter - returns builder for fluent API"""
        return StrategyBuilder(self, strategy_filter)

    def generate(self, strategy_filter: StrategyFilter, cfg: ConfigFilter) -> StrategyList:
        """Generate all strategies based on strategy_filter and apply cfg filters"""
        all_strategies = []
        
        # Use registry to generate strategies
        for filter_field, generator in self._generators.items():
            if getattr(strategy_filter, filter_field, False):
                strategies = generator.generate(cfg)
                filtered = self._filter_strategies(strategies, cfg)
                all_strategies.extend(filtered)
        
        return StrategyList(all_strategies, self)

    @staticmethod
    def _check_range(value: Optional[float], range_tuple: Optional[Tuple[float, float]]) -> bool:
        """Helper method to check if a value falls within a range."""
        if range_tuple is None:
            return True
        if value is None:
            return False
        min_val, max_val = range_tuple
        return min_val <= value <= max_val

    # ---------- INTERNAL STRATEGY FILTER ----------
    # ==================== STRATEGY-LEVEL FILTERS ====================
    def _filter_strategies(self, strategies: List, cfg: ConfigFilter) -> List:
        filtered = []

        for strategy in strategies:

            # ---- Debit Filter ----
            if cfg.debit_range and strategy.debit() > 0:
                if not self._check_range(strategy.debit(), cfg.debit_range):
                    continue

            # ---- Credit Filter ----
            if cfg.credit_range and strategy.credit() > 0:
                if not self._check_range(strategy.credit(), cfg.credit_range):
                    continue

            # ---- Metric Range Checks ----
            metrics = [
                (strategy.max_gain(),    cfg.potential_gain_range),
                (strategy.max_loss(),    cfg.potential_loss_range),
                (strategy.rr(),          cfg.rr_range),
                (strategy.net_delta(),   cfg.net_delta_range),
                (strategy.net_theta(),   cfg.net_theta_range),
                (strategy.net_vega(),    cfg.net_vega_range),
                (strategy.avg_iv(),      cfg.iv_range),
            ]

            ok = True
            for val, rng in metrics:
                if not self._check_range(val, rng):
                    ok = False
                    break

            if ok:
                filtered.append(strategy)

        return filtered

